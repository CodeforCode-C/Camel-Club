...
namespace CamelsClub.API.Controllers
{
    ...
    public class CompetitionRefereeController : BaseController
    {
        ...
        [HttpPost]
        [AuthorizeUserFilter(Role = "User")]
        [Route("api/Referee/AddNewMembers")]
        public ResultViewModel<bool> AddNewMembers(List<CompetitionCheckerPickupViewModel> viewModel)
        {
            try
            {
                var userID = int.Parse(UserID);
                return new ResultViewModel<bool>(_competitionRefereeService.AddNewMembers(viewModel, userID), Resource.DataLoaded, true);
        ...
        [HttpGet]
        [AuthorizeUserFilter(Role = "User")]
        [Route("api/Referee/GetCamelSpecificationValues")]
        public ResultViewModel<List<SpecificationValueViewModel>> GetCamelSpecificationValues(int camelCompetitionID)
        {
            try
            {
                var loggedUserID = int.Parse(UserID);
                return new ResultViewModel<List<SpecificationValueViewModel>>(_competitionRefereeService.GetCamelSpecificationValues(loggedUserID, camelCompetitionID), Resource.DataLoaded, true);

            }
            catch (Exception ex)
            {
                return new ResultViewModel<List<SpecificationValueViewModel>>(ex.Message);
            }
        }
        ...
        [HttpGet]
        [AuthorizeUserFilter(Role = "User")]
        [Route("api/Referee/GetCamelReport")]
        public ResultViewModel<FinalCamelReportViewModel> GetCamelReport(int ID)
        {
            try
            {
                var loggedUserID = int.Parse(UserID);
                return new ResultViewModel<FinalCamelReportViewModel>(_competitionRefereeService.GetCamelReport(ID, loggedUserID), Resource.DataLoaded, true);

            }
            catch (Exception ex)
            {
                return new ResultViewModel<FinalCamelReportViewModel>(ex.Message);
            }
        }

        [HttpGet]
        [AuthorizeUserFilter(Role = "User")]
        [Route("api/Referee/GetCompetitionReport")]
        public ResultViewModel<List<GroupReportViewModel>> GetCompetitionReport(int competitionID)
        {
            try
            {
                var loggedUserID = int.Parse(UserID);
                return new ResultViewModel<List<GroupReportViewModel>>(_competitionRefereeService.GetCompetitionReport(competitionID, loggedUserID), Resource.DataLoaded, true);

            }
            catch (Exception ex)
            {
                return new ResultViewModel<List<GroupReportViewModel>>(ex.Message);
            }
        }

        [HttpGet]
        [AuthorizeUserFilter(Role = "User")]
        [Route("api/Referee/GetGroupReport")]
        public ResultViewModel<GroupReportViewModel> GetGroupReport(int competitionID , int groupID)
        {
            try
            {
                var loggedUserID = int.Parse(UserID);
                return new ResultViewModel<GroupReportViewModel>(_competitionRefereeService.GetGroupReport(groupID,competitionID, loggedUserID), Resource.DataLoaded, true);

            }
            catch (Exception ex)
            {
                return new ResultViewModel<GroupReportViewModel>(ex.Message);
            }
        }
        [HttpGet]
        [AuthorizeUserFilter(Role = "User")]
