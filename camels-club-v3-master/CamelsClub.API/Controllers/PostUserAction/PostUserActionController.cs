...
namespace CamelsClub.API.Controllers.Post
{
    public class PostUserActionController : BaseController
    {
        ...
        public ResultViewModel<PagingViewModel<PostUserActionViewModel>> GetList(int postID = 0 , string orderBy = "ID", bool isAscending = false, int pageIndex = 1, int pageSize = 20)
        {
            ...
            resultViewModel.Data = _postUserActionService.Search(postID  , orderBy, isAscending, pageIndex, pageSize, Language);
        ...
        public ResultViewModel<PagingViewModel<LikeViewModel>> GetLikes(int postID , int pageIndex = 1, int pageSize = 20)
        {
            ResultViewModel<PagingViewModel<LikeViewModel>> resultViewModel = new ResultViewModel<PagingViewModel<LikeViewModel>>();

            resultViewModel.Data = _postUserActionService.GetLikes(postID, pageIndex, pageSize);
            resultViewModel.Success = true;
            resultViewModel.Message = Resource.DataLoaded;
            return resultViewModel;
        }
