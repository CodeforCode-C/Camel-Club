...
namespace CamelsClub.API.Controllers
{
    public class CamelController : BaseController
    {
        ...
        [HttpGet]
        [Route("api/Test")]
        public ResultViewModel<DateTime> Test(string orderBy = "ID", bool isAscending = false, int pageIndex = 1, int pageSize = 20)
        {
            ResultViewModel<DateTime> resultViewModel = new ResultViewModel<DateTime>();
            var x = TimeZoneInfo.FindSystemTimeZoneById("Middle East Standard Time");
            resultViewModel.Data =  TimeZoneInfo.ConvertTimeFromUtc(DateTime.UtcNow, x);
       ...
       // [AuthorizeUserFilter(Role = "User") ]
        ...
        public ResultViewModel<PagingViewModel<CamelViewModel>> GetList(int categoryId=0,string name ="", string orderBy = "ID", bool isAscending = false, int pageIndex = 1, int pageSize = 20)
        {
            ResultViewModel<PagingViewModel<CamelViewModel>> resultViewModel = new ResultViewModel<PagingViewModel<CamelViewModel>>();
            var userID = int.Parse(UserID);
            resultViewModel.Data = _camelService.Search(userID,categoryId, name ,orderBy, isAscending, pageIndex, pageSize, Language);
       ...
       // [AuthorizeUserFilter(Role = "Admin")]
        ...
        public ResultViewModel<PagingViewModel<CamelViewModel>> GetAllCamels(int userId, string name="", string orderBy = "ID", bool isAscending = false, int pageIndex = 1, int pageSize = 20)
        {
            try
            {
                ResultViewModel<PagingViewModel<CamelViewModel>> resultViewModel = new ResultViewModel<PagingViewModel<CamelViewModel>>();
                resultViewModel.Data = _camelService.Search(userId,0, name, orderBy, isAscending, pageIndex, pageSize, Language);
                resultViewModel.Success = true;
                resultViewModel.Message = Resource.DataLoaded;
                return resultViewModel;
        ...
        public ResultViewModel<PagingViewModel<CamelViewModel>> GetAllUserCamels(int userID,string name="", string orderBy = "ID", bool isAscending = false, int pageIndex = 1, int pageSize = 20)
        {
            try
            {
                ResultViewModel<PagingViewModel<CamelViewModel>> resultViewModel = new ResultViewModel<PagingViewModel<CamelViewModel>>();
