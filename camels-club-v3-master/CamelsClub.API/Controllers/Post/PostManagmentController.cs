...
namespace CamelsClub.API.Controllers.Post
{
    public class PostManagmentController : BaseController
    {
        ...
        public ResultViewModel<PagingViewModel<WholePostViewModel>> GetList(string orderBy = "ID", bool isAscending = false, int pageIndex = 1, int pageSize = 20)
        {
            ResultViewModel<PagingViewModel<WholePostViewModel>> resultViewModel = new ResultViewModel<PagingViewModel<WholePostViewModel>>();
        ...
        //[HttpPost]
        //[AuthorizeUserFilter(Role = "User")]
        //[Route("api/UserPost/Share")]
        ////[ValidateViewModel]
        //public ResultViewModel<int> SharePost(SharePostViewModel viewModel)
        //{
        //    try
        //    {
        ...
        //            var post = _postService.CreatePost(viewModel);
        ...
        [Route("api/UserPost/Share")]
        //[ValidateViewModel]
        public ResultViewModel<int> Share(SharePostViewModel view)
        {
            try
            {
                if (!ModelState.IsValid)
                ...
                {
                    view.UserID = userID;
                    var post = _postService.SharePost(view);
                    _unit.Save();
                    resultViewModel.Success = true;
                    resultViewModel.Data = post.ID;
                    resultViewModel.Message = Resource.PostAddedSuccessfully;
