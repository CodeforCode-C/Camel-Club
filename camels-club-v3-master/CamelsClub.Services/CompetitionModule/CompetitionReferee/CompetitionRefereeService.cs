...
namespace CamelsClub.Services
{
    public class CompetitionRefereeService : ICompetitionRefereeService
    {
        ...
        public bool ManualAllocate(List<RefereeAllocateCreateViewModel> viewModels)
        {
            ...
            {
                throw new Exception("لم تكتمل مرحلة التمييز بعد");
            }
        ...
        public bool AutoAllocate(CheckerJoinCompetitionCreateViewModel viewModel)
        {
            ...
            {
                throw new Exception("تم التوزيع بالفعل");
            }
        ...
        public bool JoinCompetition(CheckerJoinCompetitionCreateViewModel viewModel)
        {
            ...
            data.Referee.JoinDateTime = DateTime.UtcNow;
            _unit.Save();

            if (data.Referee.IsBoss)
            {
                //send push notification to owner to notify him
                var notification = new NotificationCreateViewModel
                {
                    ContentArabic = $"{NotificationArabicKeys.JoinCompetitionAsReferee} {data.Competition.NameArabic}",
                    ContentEnglish = $"{NotificationEnglishKeys.JoinCompetitionAsReferee} {data.Competition.NamEnglish}",
                    NotificationTypeID = (int)TypeOfNotification.JoinCompetition,
                    ArbNotificationType = "اشتراك عضو الي لجنة تحكيم المسايقة",
                    EngNotificationType = "Join the competition as Referee",
                    SourceID = data.Referee.UserID,
            ...
            //if(joinedReferees == data.Competition.MinimumRefereesCount)
            //{
            //    var notification = new NotificationCreateViewModel
            //    {
            //        ContentArabic = $"{NotificationArabicKeys.CompletedRefereesMinimumCount} {data.Competition.NameArabic}",
            //        ContentEnglish = $"{NotificationEnglishKeys.CompletedRefereesMinimumCount} {data.Competition.NamEnglish}",
            //        NotificationTypeID = (int)TypeOfNotification.JoinCompetition,
            //        ArbNotificationType = $"{NotificationArabicKeys.CompletedRefereesMinimumCount} {data.Competition.NameArabic}",
            //        EngNotificationType = $"{NotificationEnglishKeys.CompletedRefereesMinimumCount} {data.Competition.NamEnglish}",
        ...
        public bool PickupTeam(List<CompetitionCheckerPickupViewModel> viewModels, int loggedUserID)
        {
            ...
            if (viewModels != null && viewModels.Count() % 2 == 0)
            {
                throw new Exception("يجب ان يكون عدد الحكام فردي");
            }

            if (viewModels != null && viewModels.Count() == 1)
            {
                throw new Exception("لا يمكن اعتماد حكم واحد فقط");
            }
        ...
        public bool AddNewMembers(List<CompetitionCheckerPickupViewModel> viewModels, int loggedUserID)
        {
            ...
            if (joinedCount + IDs.Count != competition.MinimumRefereesCount)
            {
                throw new Exception($"يجب اختيار العدد المحدد");
            }

            data.ForEach(x => x.Referee.PickupDateTime = DateTime.UtcNow);
            _unit.Save();
            //send notification to picked checkers
            var notification = new NotificationCreateViewModel
            {
            ...
            _notificationService.SendNotifictionForReferees(notification, data.Select(x => new CompetitionRefereeCreateViewModel { UserID = x.Referee.UserID }).ToList());
            return true;
        }
        ...
        {
            //check all values are greater than zerrrooooooo
            if(viewModel.CamelsSpecificationValues.Any(x=>x.SpecificationValue == 0))
            {
                throw new Exception("يجب الا تقوم بادخال اي قيم ب صفر");
            }
            //get competitonId 
            var competitionID = _camelCompetitionRepository.GetById(viewModel.CamelCompetitionID).CompetitionID;
            if (!_camelCompetitionRepository.GetAll()
        ...
        public bool ApproveGroup(ApproveGroupCreateViewModel viewModel)
        {
            if (!_camelCompetitionRepository.GetAll()
               ...
               .All(x => x.ApprovedByCheckerBossDateTime != null || x.RejectedByCheckerBossDateTime != null))
            {
                throw new Exception("لم يكتمل التمييز بعد");
            }
            ...
            if (_camelCompetitionRepository.GetAll()
                    ...
                    .All(x => x.ApprovedByRefereeBossDateTime != null))
            {
                throw new Exception("لقد تم تقييم المنقية من قبل");
            }
            ...
            {
                throw new Exception("لم يتم انتهاء مرحلة لجان التحكيم");
            }
            //approve all camels
        ...
        //boss only
        public List<GroupReportViewModel> GetCompetitionReport(int competitionID, int loggedUserID)
        {
            //get groups in that competition
            var groupIDs =
                _camelCompetitionRepository.GetAll()
                .Where(x => x.ApprovedByCheckerBossDateTime != null)
                .Where(x => x.CompetitionID == competitionID)
            ...
            var res = new List<GroupReportViewModel>();
            foreach (var item in groupIDs)
            {
                var groupReport = GetGroupReport(item, competitionID, loggedUserID);
                res.Add(groupReport);
            }
            return res;
        }
        ...
        //boss only
        public GroupReportViewModel GetGroupReport(int groupID,int competitionID, int loggedUserID)
        {
            string protocol = HttpContext.Current.Request.Url.Scheme.ToString();
            string hostName = HttpContext.Current.Request.Url.Authority.ToString();

            var camelCompetitionIDs =
              _camelCompetitionRepository.GetAll()
              .Where(x => x.GroupID == groupID)
              .Where(x => x.ApprovedByCheckerBossDateTime != null)
              .Where(x => x.CompetitionID == competitionID)
              .Select(x => x.ID);
            var res = new List<FinalCamelReportViewModel>();
            foreach (var item in camelCompetitionIDs)
            {
               var report =  GetCamelReport(item, loggedUserID);
                res.Add(report);
            }
            //get group data 
            var group = _groupRepository.GetById(groupID);
            var groupReport = new GroupReportViewModel
            {
                GroupID = groupID,
                GroupNameArabic = group.NameArabic,
                GroupNameEnglish = group.NameEnglish,
                GroupImage = protocol + "://" + hostName + "/uploads/Camel-Document/" + group.Image,
                Camels = res
            };
            return groupReport;
        }
        //for boss
        public FinalCamelReportViewModel GetCamelReport(int ID, int loggedUserID)
        {
            string protocol = HttpContext.Current.Request.Url.Scheme.ToString();
            string hostName = HttpContext.Current.Request.Url.Authority.ToString();
            //get ompetition
            ...
            if (boss.UserID == loggedUserID)
            {
               ...
               .Select(x => new CamelCompetitionSpecificationBossViewModel
               {
                ...
                var res = new List<CamelReportViewModel>();
                foreach (var item in camel.RefereeEvaluates)
                {
                    res.Add(new CamelReportViewModel
                    {
                        RefereeUserName = item.UserName,
                        RefereeDisplayName = item.DisplayName,
                        RefereeImage = item.UserImage,
                        TotalValue = Math.Round(item.Vaules.Sum(v => v.SpecificationValue),2)
                    });

                }
                var finalResult = new FinalCamelReportViewModel
                {
                    Referees = res,
                    CamelImages = camel.CamelImages,
                    CamelName = camel.CamelName,
                    camelID = camel.ID,
                    TotalValue = Math.Round(res.Sum(r => r.TotalValue) / res.Count())
                };
                return finalResult;
            }
        ...
        public List<CamelSpecificationViewModel> GetRefereeCamelSpecifications(int ID, int loggedUserID)
        {
            ...
            else
            {
               ...
               .Select(g => new CamelSpecificationViewModel
               {
                   ...
                   MaxValue = g.CamelCompetition.Competition.CompetitionSpecifications.FirstOrDefault(s => s.CamelSpecificationID == g.CamelSpecificationID).MaxAllowedValue
               }).ToList();
                ...
                var data = _competitionRepository.GetAll()
                   ...
                   .Select(x => new
                   {
                       CamelsCount = x.CamelCompetitions.Where(c => c.CompetitionID == competitionID)
                                    ...
                                    .Count(),
                       Referees = x.CompetitionReferees.Where(r=> r.PickupDateTime != null && r.RejectDateTime == null).Where(c => !c.IsBoss).Select(c => new CompetitionRefereeViewModel
                       {
                           ID = c.ID,
        ...
        public RefereesReportViewModel GetTeam(CheckerJoinCompetitionCreateViewModel viewModel)
        {
                        ...
                        .Any())
            {
                var data = _competitionRepository.GetAll()
                   ...
                   .Select(x => new
                   {
                       MaximumRefereesCount = x.MaximumRefereesCount,
                       MinimumRefereesCount = x.MinimumRefereesCount,
                       IsTeamPicked = x.RefereePickupTeamDateTime != null,
                       CamelsCount = x.CamelCompetitions.Where(c => c.CompetitionID == viewModel.CompetitionID)
                                    .Where(c => c.ApprovedByCheckerBossDateTime != null)
                                    .Count(),
                       Referees = x.CompetitionReferees.Where(c =>!c.IsDeleted  &&!c.IsBoss).Select(c => new CompetitionRefereeViewModel
                       {
                           ID = c.ID,
                           UserName = c.User.UserName,
                           DisplayName = c.User.DisplayName,
                           ...
                           HasJoined = c.JoinDateTime.HasValue,
                           HasPicked = c.PickupDateTime.HasValue,
                           HasRejected = c.RejectDateTime != null,
                           CamelsEvaluatedCount = c.Competition.CamelCompetitions.Where(cc => cc.RefereeReviews.Where(a => a.CompetitionRefereeID == c.ID).Any())
                                                 .Count(),
                           AssignedCamels = c.Allocates.Select(a => a.Group).Select(g => g.CamelGroups).Count()
                       }).ToList(),
                ...
                var res = new RefereesReportViewModel
                {
                    ...
                    IsAnyMemberRejected = data.Referees.Any(x => x.HasRejected),
                    AllRefereesAccepted = data.Referees.Where(x => x.HasJoined).Count() == data.MinimumRefereesCount,
                    IsReplaceAllowed = data.Referees.Where(x => x.HasPicked && !x.HasRejected).Count() < data.MinimumRefereesCount,
                    TotalNumberOfEvaluatedCamels = 0,
                    CheckingCompletitionRatio = 0,
                    MaximumRefereesCount = data.MaximumRefereesCount,
                    MinimumRefereesCount = data.MinimumRefereesCount,
                    Referees = data.Referees.ToList()
                };
            ...
            var refereesInCompetition =
                _groupRepository.GetAll()
                        ...
                        .Select(g => new CompetitionRefereeViewModel
                        {
                            ...
                            HasJoined = g.Key.JoinDateTime.HasValue,
                            HasPicked = g.Key.PickupDateTime.HasValue,
                            HasRejected = g.Key.RejectDateTime != null,
        ...
        public bool RejectCompetition(CheckerJoinCompetitionCreateViewModel viewModel)
        {
            ...
            if (data.Referee.JoinDateTime.HasValue)
            {
                throw new Exception("You joined , you can not reject ");
            }
            else
            {
                data.Referee.RejectDateTime = DateTime.UtcNow;
                if (data.Referee.IsBoss)
                {
                    data.Referee.IsDeleted = true;
                }
                _unit.Save();
            }
            if (data.Referee.IsBoss)
            {
                ...
                var notification = new NotificationCreateViewModel
                {
                    ContentArabic = $"{NotificationArabicKeys.RejectCompetitionAsReferee} {data.Competition.NameArabic}",
                    ContentEnglish = $"{NotificationEnglishKeys.RejectCompetitionAsReferee} {data.Competition.NamEnglish}",
                    NotificationTypeID = data.Referee.IsBoss ? (int)TypeOfNotification.RejectCompetitionByRefereeBoss : (int)TypeOfNotification.RejectCompetitionByReferee,
                    ArbNotificationType = $"{NotificationArabicKeys.RejectCompetitionAsReferee} {data.Competition.NameArabic}",
                    EngNotificationType = $"{NotificationEnglishKeys.RejectCompetitionAsReferee} {data.Competition.NamEnglish}",
                    SourceID = data.Referee.UserID,
                    DestinationID = data.Competition.UserID,
                    //   SourceName = insertedCompetition.User.Name,
                    CompetitionID = data.Competition.ID,
        ...
        {
            if(_competitionRepository.GetById(viewModel.CompetitionID).Completed != null)
            {
                throw new Exception("لقد تمت عملية ارسال النتائج الي منشى المسابقة");
            }
            ...
            var approvedGroupsFromRefereeing =
                ...
                .Where(x => x.ApprovedByRefereeBossDateTime != null)
                .Count();
            //confirm all inviters has final score now
            //var allHasFinalScore =
            //    _competitionInviteRepository.GetAll()
            //    .Where(x => x.CompetitionID == viewModel.CompetitionID)
            //    .Where(x => x.CamelCompetitions.Where(cc=>cc.CompetitionID == viewModel.CompetitionID).All(cc => cc.ApprovedByCheckerBossDateTime != null))
            //    .All(x => x.FinalScore != null);
            //if (!allHasFinalScore)
            //{
            //    throw new Exception("لم يكتمل تحكيم كل المنقيات بعد");
            //}
            if(approvedGroupsFromChecking != approvedGroupsFromRefereeing)
            {
                throw new Exception($"لم يتم اعتماد المنقيات بعد");
            }
        ...
        public List<SpecificationValueViewModel> GetCamelSpecificationValues(int userID, int camelCompetitionID)
        {
            return _camelCompetitionRepository.GetAll().Where(x => x.ID == camelCompetitionID)
                .SelectMany(x => x.RefereeReviews)
                .Where(r => r.CompetitionReferee.UserID == userID)
                .Select(r => new SpecificationValueViewModel
                {
                   CamelSpecificationID =  r.CamelSpecificationID ,
                   ActualPercentageValue =  r.ActualPercentageValue,
                   SpecificationArabic =  r.CamelSpecification.SpecificationArabic,
                    SpecificationEnglish =  r.CamelSpecification.SpecificationEnglish
    ...
    public class GroupReportViewModel
    {
        ...
        public List<FinalCamelReportViewModel> Camels { get; set; }
    }

    public class FinalCamelReportViewModel
    {
        public List<CamelReportViewModel> Referees { get; set; }
        public double TotalValue { get; set; }
        public string CamelName { get; internal set; }
        public int camelID { get; set; }
        public List<CamelDocumentViewModel> CamelImages { get; internal set; }
    }

    public class CamelReportViewModel
    {
        public string RefereeUserName { get; set; }
        public string RefereeDisplayName { get; set; }
