...
namespace CamelsClub.Services
{
    public interface IUserService
    {
        ...
        PagingViewModel<UserSearchViewModel> FindAvailableUsersToBeReferees(int competitionID, int userID, string text, string orderBy = "ID", bool isAscending = false, int pageIndex = 1, int pageSize = 20, Languages language = Languages.Arabic);
        PagingViewModel<UserSearchViewModel> FindAvailableUsersToBeCheckers(int competitionID, int userID, string text, string orderBy = "ID", bool isAscending = false, int pageIndex = 1, int pageSize = 20, Languages language = Languages.Arabic);
        bool IsExistUser(int userId);
        bool SignOut(int userId,string token);
        PagingViewModel<UserSearchViewModel> FindUsers(int userID , string text, string orderBy = "ID", bool isAscending = false, int pageIndex = 1, int pageSize = 20, Languages language = Languages.Arabic, List<int> excludedUderIDs = null);
