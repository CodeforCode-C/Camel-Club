...
namespace CamelsClub.Services
{
    public class UserProfileService : IUserProfileService
    {
        ...
        public UserProfileViewModel GetByID(int loggedUserID, int userId)
        {
            ...
            {
                userID1 = userId;
                userID2 = loggedUserID;
            }
            // get relation
            var friendRequest = _friendRequestRepository.GetAll().Where(x => x.FromUserID == userID1 && x.ToUserID == userID2 && x.Status != (int)FriendRequestStatus.NoFriendRequest).FirstOrDefault();
            ...
            var userProfileData = _userRepository.GetAll()
                                ...
                                .Select(obj => new UserProfileViewModel
                                {
                                    ...
                                    NumberOfShare = obj.Posts.Where(p=> !p.IsDeleted && p.ParentPostID != null).Count(),
                                    NumberOfCompetitionsJoined = obj.CompetitionInvites
                                                            .Where(x => !x.IsDeleted && x.SubmitDateTime != null)
                                                            .Count(),
                                    NumberOfFriends = obj.FromFriendRequests.Where(x => !x.IsDeleted && x.Status == (int)FriendRequestStatus.Approved).Count() +
                                                      obj.ToFriendRequests.Where(x => !x.IsDeleted && x.Status == (int)FriendRequestStatus.Approved).Count(),
                                    ...
                                    Posts = obj.Posts.Where(p=>!p.IsDeleted).Select(p=> new PostDetailsViewModel
                                    {
                                        ...
                                        CommentsCount = p.Comments.Where(com => !com.IsDeleted).Count(),
            ...
            userProfileData.Posts.ToList().ForEach(x =>
            {
                if (x.ParentPostID.HasValue)
                {
                    var parentPost = _postRepository.GetAll().Where(p => p.ID == x.ParentPostID).Select(p => new
                    {
                        UserID = p.UserID,
                        UserName = p.User.UserName,
                        Text = p.Text,
            ...
            {
                userProfileData.Status = friendRequest.Status;
                userProfileData.ActionByLoggedUser = loggedUserID == friendRequest.ActionBy;
        ...
        public UserProfileViewModel GetMyProfile(int userId)
        {
            ...
            var userProfileData = _userRepository.GetAll()
                                ...
                                .Select(obj => new UserProfileViewModel
                                {
                                    ...
                                    Posts = obj.Posts.Where(p => !p.IsDeleted && (!p.ParentPostID.HasValue || p.ParentPostID.HasValue && !p.ParentPost.IsDeleted)).OrderByDescending(p=>p.ID).Skip(0).Take(20).Select(p => new PostDetailsViewModel
                                    {
                                        ID = p.ID,
                                        Text = p.Text,
                                        UserID = p.UserID,
                                        ...
                                        Comments = p.Comments.Where(com => !com.IsDeleted).OrderByDescending(c => c.ID).Skip(0).Take(20).Select(com => new CommentViewModel
                                        {
                                    ...
                                    Camels = obj.Camels.Where(c=> !c.IsDeleted).OrderByDescending(p => p.ID).Skip(0).Take(20).Select(x => new CamelViewModel
                                    {
                                        ID = x.ID,
                                        BirthDate = x.BirthDate,
                                        CategoryArabicName = x.Category.NameArabic,
        ...
        public void Test()
        {
           var users =  _userRepository.GetAll().ToList();
            ...
            {
                //item.Phone = SecurityHelper.Decrypt(item.Phone);
            }
            _unit.Save();
        }
