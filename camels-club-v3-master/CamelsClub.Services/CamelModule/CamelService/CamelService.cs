...
namespace CamelsClub.Services
{
    public class CamelService : ICamelService
    {
        ...
        public PagingViewModel<CamelViewModel> Search(int userID, int categoryId=0, string name="", string orderBy = "ID", bool isAscending = false, int pageIndex = 1, int pageSize = 20, Languages language = Languages.Arabic)
        {
            ...
            var query = _repo.GetAll().Where(post => !post.IsDeleted)
                ...
                .Where(x => name == "" || x.Name.Contains(name));
        ...
        public void Edit(CamelCreateViewModel viewModel)
        {
            ...
            if (_camelCompetitionRepository.GetAll()
                .Any(cc=> cc.Competition.Published == null && cc.Group.IsLocked && cc.CamelID == viewModel.ID))
            {
                throw new Exception("الناقة موجودة بمسابقة جارية ولا يمكنك التعديل");
            }
        ...
        {
            var isCamelInCurrentCompetition = _repo.GetAll().Where(x => x.ID == id).Any(x => x.CamelCompetitions.Any(cc => cc.Competition.Published == null));
            if (isCamelInCurrentCompetition)
            {
                throw new Exception($"الجمل مشترك في احدي المسسابقات الجارية ولا يمكنك المسح");
            }
            _repo.Remove(id);
        }
