...
namespace CamelsClub.Services
{
    public class GroupService : IGroupService
    {
        ...
        public List<CamelViewModel> FilterCamels(int groupID, string text) 
        {
            return 
            _repo.GetAll().Where(x => x.ID == groupID)
                                        .SelectMany(x => x.CamelGroups)
        ...
        {
            var isGroupInCurrentCompetition = _repo.GetAll().Where(x => x.ID == id).SelectMany(x => x.CamelCompetitions).Any(cc => cc.Competition.Published == null);
            if (isGroupInCurrentCompetition)
            {
                throw new Exception($"المنقية مشتركة في احدي المسابقات الجاريه ولا يمكنك المسح");
            }
            _repo.Remove(id);
        }
        ...
        {
            if (_repo.GetById(groupID).IsLocked)
            {
                throw new Exception("المنقية مشتركة في مسابقة ولا يمكنك المسح");
            }
