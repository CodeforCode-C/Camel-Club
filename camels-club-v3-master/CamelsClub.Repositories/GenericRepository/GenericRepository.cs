...
namespace CamelsClub.Repositories
{
    public class GenericRepository<T> where T : BaseModel
    {
        ...
        public T Add(T obj)
        {
            ...
            var saudiZone = TimeZoneInfo.FindSystemTimeZoneById("Middle East Standard Time");
            obj.CreatedDate = TimeZoneInfo.ConvertTimeFromUtc(DateTime.UtcNow, saudiZone);
        ...
        public virtual T Edit(T entity)
        {
            if (defaultExcludedEditProperties.Any())
            {
                ...
                var saudiZone = TimeZoneInfo.FindSystemTimeZoneById("Middle East Standard Time");
                entity.UpdateDate = TimeZoneInfo.ConvertTimeFromUtc(DateTime.UtcNow, saudiZone);
            }
            else
            {
                var saudiZone = TimeZoneInfo.FindSystemTimeZoneById("Middle East Standard Time");
